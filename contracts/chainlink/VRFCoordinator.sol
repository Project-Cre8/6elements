...
contract VRF {
  ...
  uint256 private constant GROUP_ORDER = // solium-disable-next-line indentation // Number of points in Secp256k1
  0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364141;
  // Prime characteristic of the galois field over which Secp256k1 is defined
  uint256 private constant FIELD_SIZE = // solium-disable-next-line indentation
  ...
  */
   ...
   */
  function randomValueFromVRFProof(bytes memory proof) internal view returns (uint256 output) {
    ...
    (pk, gamma, cSSeed, uWitness, cGammaWitness, sHashWitness, zInv) = abi.decode(
      proof,
      (uint256[2], uint256[2], uint256[3], address, uint256[2], uint256[2], uint256)
    );
    verifyVRFProof(
      pk,
      gamma,
...
contract VRFCoordinator is VRF, VRFRequestIDBase {
  ...
  function registerProvingKey(
    ...
    require(_fee <= 1e9 ether, "you can't charge more than all the LINK in the world, greedy");
  ...
  function fulfillRandomnessRequest(bytes memory _proof) public {
    (bytes32 currentKeyHash, Callback memory callback, bytes32 requestId, uint256 randomness) = getRandomnessFromProof(
      _proof
    );
